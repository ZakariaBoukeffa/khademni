...
class CompanyProfile {
    ...
    public function __construct($dbhost, $dbuser, $dbpass, $dbname) {
        ...
        $this->updateAboutSection($email);
    }
    
    private function updateAboutSection($email) {
        if (isset($_POST['about'])) {
            $about = $_POST['about'];
            
            // Update the company's about field in the database
            $update_sql = "UPDATE companies SET about='$about' WHERE email='$email'";
            
            // Execute the update query
            if (mysqli_query($this->conn, $update_sql)) {
                header("Location: company-profile-index.php");
                exit; // Exit the script to prevent further execution
            } else {
                echo "Error updating about section: " . mysqli_error($this->conn);
            }
        }
    }
    
    public function fetchCompanyData($email) {
        // Fetch company data from database based on email
...
$companyProfile = new CompanyProfile($dbhost, $dbuser, $dbpass, $dbname);

$email = $_SESSION['email'];
$companyData = $companyProfile->fetchCompanyData($email);

if ($companyData) {
    $cname = $companyData['company_name'];
    $hname = $companyData['head_name'];
    $email = $companyData['email'];
    $about = $companyData['about'];
    $field = $companyData['field'];
    
    // Display the company profile data and form for updating about section
    ?>
...
$companyProfile->closeConnection();
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <div class="col col-lg-50 mb-50 mb-lg-50">
        <div class="card mb-5" style="border-radius: .5rem;">
          ...
          <div class="col-md-4 gradient-custom text-center text-white"
              ...
              <img src="company.png" alt="Avatar" class="img-fluid my-5" style="width: 100px; height:100px ;" />
                
            </div>
            <div class="col-md-8">
              <div class="card-body p-4">
              <form action="company-edit-profile.php" method="post">
                    <label for="about">About:</label>
                    <br>
